...
function PlayListGenerator_popular(playlist_duration, songs, n) {
	// Base Case
	if (playlist_duration === 0 || n < songs.length) return 0;
	...
	else new_playlist.push(songs[n][0]); //add song to new playlist array
	return max(
		PlayListGenerator_popular(
			playlist_duration - songs[n - 1][1],
			songs,
			n - 1
		),
...
audio features, returns an array of choosen song ids
*/
export function PlaylistGenerator_features(playlist_duration, songs, features) {
    ...
    while(x < songs.length) {
        ...
        else if (features.indexOf("speech") !== -1 && (songs[x][6] > 0.66 || songs[x][6] < 0.33)) {
            delete songs[x];
        }
